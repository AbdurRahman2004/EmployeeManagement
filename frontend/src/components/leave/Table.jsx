...
const Table = () => {
    ...
    const fetchLeaves = async () => {
        try {
            const response = await axios.get("https://employee-api-eight.vercel.app/api/leave", {
              headers: {
                "Authorization": `Bearer ${localStorage.getItem('token')}`
              }
            });
            console.log("Fetched employees: ", response.data.leaves);
  ...
  <div className='flex justify-between items-center mt-3'><input type="text" className='px-4 py-0.5' 
  onChange={ filterByInput} placeholder='Search By Emp Id' />
